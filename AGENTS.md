...
This repository uses the `angularCliKikita` MCP server. Do not use the generic
`angularCli` server for this project; that server points at the older
`discord-bot` workspace.

Use `angularCliKikita.list_projects` first when Angular workspace context
matters. The current Angular CLI MCP tools `get_best_practices` and
