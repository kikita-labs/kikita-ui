...
<kui-popover #myPop placement="bottom" [arrow]="true">
...
<kui-popover #confirmPop placement="bottom" align="start" [arrow]="true">
...
<kui-popover #hoverPop placement="top" triggerType="hover" [arrow]="true">
...
```html
<kui-popover [(open)]="isOpen" placement="bottom">...</kui-popover>
```
...
- **Flip:** CDK `FlexibleConnectedPositionStrategy` — prefers stated `placement`, flips to opposite side if needed. `align` is preserved after flip.
...
- **Accessibility:** `role="dialog"` on panel, `aria-expanded` on trigger, ESC returns focus to trigger.
